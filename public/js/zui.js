...
define(['jquery'],function($){
	(function($){
		$.zui={indexOfArray:function(a,o,id){
		...
		_115(_116,_117,flag);
		if(opts.deepCheck){
		$.zui.forEach(_117.children||[],true,function(n){
		_115(_116,n,flag);
		});
		}else{
		var _119=[];
		...
		var c1=0;
		var len=0;
		$.zui.forEach(row.children||[],false,function(r){
		if(r.checkState){
		len++;
		...
		$.zui.forEach(data,true,function(node){
		_174.push(_171(node));
		});
		return _174;
		};
		...
		var _17a=[];
		$.zui.forEach($.data(_178,"tree").data,true,function(n){
		if(n.checkState&&$.zui.indexOfArray(_179,n.checkState)!=-1){
		_17a.push(_171(n));
		}
		});
		return _17a;
		...
		$.zui.forEach(data.children,true,function(node){
		_171(node);
		});
		}
		return data;
		...
		var ids={};
		$.zui.forEach(_19b.data,true,function(node){
		...
		if(item.checked){
		$.zui.addArrayItem(_1c2.tmpIds,item.domId);
		}
		}
		...
		$.fn.calendar.defaults={width:180,height:180,fit:false,border:true,showWeek:false,firstDay:0,weeks:["一", "二", "三", "四", "五", "六", "日"], months:["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],year:new Date().getFullYear(),month:new Date().getMonth()+1,current:(function(){
		var d=new Date();
		...
		var _645=0;
		function _646(a,o){
		return $.zui.indexOfArray(a,o);
		};
		function _647(a,o,id){
		$.zui.removeArrayItem(a,o,id);
		};
		function _648(a,o,r){
		$.zui.addArrayItem(a,o,r);
		};
		...
		var idx1=$.zui.indexOfArray(rows,opts.idField,opts.lastSelectedIndex);
		var idx2=$.zui.indexOfArray(rows,opts.idField,_913);
		...
		ck.addClass("tree-checkbox"+flag);
		if(flag==0){
		$.zui.removeArrayItem(_924,opts.idField,row[opts.idField]);
		}else{
		$.zui.addArrayItem(_924,opts.idField,row);
		}
		};
		function _91f(_925,row,_926){
		var flag=_926?1:0;
		_921(_925,row,flag);
		$.zui.forEach(row.children||[],true,function(r){
		_921(_925,r,flag);
		});
		};
		...
		var _957=null;
		$.zui.forEach(_956.data,true,function(node){
		...
		$.zui.removeArrayItem(_980.checkedRows,opts.idField,_97f);
		_90e(_97e);
		if(prow){
		...
		_997=_997||"checked";
		var rows=[];
		$.zui.forEach(jq.data("treegrid").checkedRows,false,function(row){
		...
		var flag=0;
		var crow=$.zui.getArrayItem(_9b3.checkedRows,opts.idField,row[opts.idField]);
		if(crow){
		flag=crow.checkState=="checked"?1:2;
		row.checkState=crow.checkState;
		row.checked=crow.checked;
		$.zui.addArrayItem(_9b3.checkedRows,opts.idField,row);
		}else{
		var prow=$.zui.getArrayItem(_9b3.checkedRows,opts.idField,row._parentId);
		...
		if(row.checked){
		$.zui.addArrayItem(_9b3.tmpIds,row[opts.idField]);
		}
		}
		...
		return $.zui.indexOfArray(_a53.data,_a53.options.valueField,_a52);
		};
		...
		if(row){
		if($.zui.indexOfArray(_a6b,row[opts.valueField])==-1){
		...
		$.zui.forEach(_a70.data,false,function(row){
		if(row["selected"]){
		$.zui.addArrayItem(vv,row[opts.valueField]+"");
		}
		});
		if(opts.multiple){
		_a61(_a6e,vv,_a6f);
		...
		var _ab1=$.zui.indexOfArray(_ab0.groups,"value",_aaf);
		...
		var item=$.zui.getArrayItem(a,"id",_ad1);
		...
		$.zui.addArrayItem(opts.mappingRows,"id",_adf);
		return _adf.id;
		}else{
		return _adf;
		}
		...
		var item=$.zui.getArrayItem(a,opts.idField,_b0e);
		...
		if($.zui.indexOfArray(opts.mappingRows,opts.idField,v)>=0){
		$.zui.addArrayItem(vv,v);
		}
		});
		...
		$.zui.addArrayItem(opts.mappingRows,opts.idField,_b20);
		...
		$.zui.addArrayItem(_b2b,row[opts.idField]);
		});
		}
		_b30(_b25,_b2b);
		...
		var item=$.zui.getArrayItem(a,opts.idField,_b39);
		...
		if(q){
		var v=undefined;
		$.zui.forEach(data,true,function(row){
		...
		if(v==undefined){
		$.zui.forEach(opts.mappingRows,false,function(row){
		...
		$.zui.addArrayItem(opts.mappingRows,opts.idField,_b45);
