...
define(['jquery','functions'],function($,_f) {
    ...
    var ip_column = [[
        ...
        {
            field: 'active', title: '', width: 30, align: 'left', formatter: function (value, row, index) {
            if (value == 1) {
            } else {
        ...
        {
            field: 'connected', title: '状态', sortable: true, align: 'center', formatter: function (valve, row, index) {
            if (valve == 1) {
                return "<span data-id=" + valve + ">正常</span>";
            } else if (valve == 0) {
                return "<span data-id=" + valve + ">断开连接</span>";
            } else if (valve == 2) {
                return "<span data-id=" + valve + ">警告,负载过重</span>";
            } else {
                return "<span data-id=" + valve + ">未知</span>";
            }
        }
        },
